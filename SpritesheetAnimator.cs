...
public class SpritesheetAnimator : MonoBehaviour {
	...
	private Rect[] textureUV;
	private Vector2[] originalUV;
	private int counter = 0;
	public Material material;
	public TextAsset uvMap;
	public float framesPerSecond;
	public bool pause = false;
	// Initialization code...
	void Start() {
		// Retrieve mesh from active object and copy its original UV map
		objectMesh = GetComponent<MeshFilter>().mesh;
		originalUV = new Vector2[objectMesh.uv.Length];
		objectMesh.uv.CopyTo(originalUV, 0);
		// Read the UV maps from file
		textureUV = readUVMap();
		gameObject.renderer.material = material;
		// Start the update (loop) co-routine
		StartCoroutine("CoUpdate");
	}
	// Co-routine that runs the frame-by-frame sprite animation.
	...
	IEnumerator CoUpdate() {
		while (true) {
			...
			// Start the sprite animation co-routine
